...
include_once("includes/form_functions.php");
	// START FORM PROCESSING
		...
		$cust_id = trim(mysql_prep($_POST['cust_id']));
		$date=$year."-".$month."-".$day;
		echo $policy_no. $date ;
		...
		if ( empty($errors) ) {
			...
			$query = "INSERT INTO policy (
							...
							'{$policy_no}', '{$agent_id}',  '{$type}', '{$date}', '{$term_price}', '{$deductable}', '{$coverage}', '{$cust_id}' 
						)";
			$result = mysql_query($query, $connection);
			
			if ($result) {
				switch($type) {
	case "Accident" : redirect_to("car_reg.php?policyno={$policy_no}&agentid={$agent_id}"); break;
	case "Fire" : redirect_to("house_reg.php?policyno={$policy_no}&agentid={$agent_id}"); break;
	case "Mediclaim" : redirect_to("medi.php?policyno={$policy_no}&agentid={$agent_id}"); break;
				}
...
left:+106px">This policy will be added for the agent name  ame   with agent ID <strong style="color:green"><?php echo $agent_id;  ?> </strong></p>
      <p align="left"><?php if(isset($message))echo $message; ?>&nbsp;</p>
   
          
		
			<form id="form2" name="form2" method="post" action="select_agent.php<?php echo "?agentid=".$agent_id."&name=".$name ?>">
			  <table width="731" border="0" cellspacing="10">
			    ...
			    <tr>
                  ...
                  $no_of_cust="SELECT cust_id,name FROM customer";
				$cust_r = mysql_query($no_of_cust, $connection);
				  
				  while($row = mysql_fetch_array($cust_r))
	{
