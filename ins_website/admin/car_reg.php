...
include_once("includes/form_functions.php");
	// START FORM PROCESSING
		...
		$required_fields = array('licence_plate', 'make', 'month', 'day', 'year', 'model_no', 'value');
		$errors = array_merge($errors, check_required_fields($required_fields, $_POST));



		$licence_plate = trim(mysql_prep($_POST['licence_plate']));

		$make = trim(mysql_prep($_POST['make']));
		$month = trim(mysql_prep($_POST['month']));
		$day = trim(mysql_prep($_POST['day']));
		$year = trim(mysql_prep($_POST['year']));
		$model_no = trim(mysql_prep($_POST['model_no']));
		...
		if ( empty($errors) ) {
			...
			$query = "INSERT INTO car (
							licence_plate, purchase_date ,  make, model_no,  policy_no, value, description
						) VALUES (
							'{$licence_plate}', '{$pdate}',  '{$make}', '{$model_no}',  '{$policy_no1}', '{$value}' ,'{$description}'
						)";
			...
			if ($result) {
				...
				if(isset($_GET['agentid']))
					redirect_to("new_policy.php?policyno={$policy_no1}");
				else 
					redirect_to("new_policy_cust.php?policyno={$policy_no1}");
...
<p align="left">
  ...
  &nbsp;</p>
<form id="form2" name="form2" method="post" action= "<?php echo "car_reg.php?policyno=".$policy_no1; ?>">
<p>&nbsp;</p>

  <table width="731" border="0" cellspacing="10">
    <tr>
      <th width="86" scope="row">&nbsp;</th>
      <th width="154" scope="row"><div align="left">Licence Plate No</div></th>
      <td width="236"><label for="textfield2"></label>
        <input type="text" name="licence_plate" id="textfield2" /></td>
      <td width="182">&nbsp;</td>
    </tr>
    <tr>
      <th scope="row">&nbsp;</th>
      <th scope="row"><div align="left">Purchase Date</div></th>
    ...
    <tr>
      <th scope="row">&nbsp;</th>
      <th scope="row"><div align="left">Maker</div></th>
      <td><input type="text" name="make" id="textfield2" /></td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <th scope="row">&nbsp;</th>
      <th scope="row"><div align="left">Model Number</div></th>
      <td><input type="text" name="model_no" id="textfield3" /></td>
      <td>&nbsp;</td>
    </tr>
    <tr>
    ...
    <tr>
      <th scope="row">&nbsp;</th>
      <th scope="row"><div align="left">Value</div></th>
      <td><input type="text" name="value" id="textfield4" /></td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <th scope="row">&nbsp;</th>
