...
$agent_id=$_SESSION['agent_id'];
	if(isset($_POST['submit_policy']))
	{
		...
		$query_payment="INSERT INTO payment VALUES ('{$polino}', {$premium}, '{$date}', 'Pending')  ";
		$result_set_pol = mysql_query($query_payment);
		confirm_query($result_set_pol);
		if($result_set_pol)
		{
			$policy_updated="the policy has been Extended";	
		}
		else
	...
	if (count($errors) == 1) {
		...
		$query .= "(select c1.policy_no from payment c1 WHERE status='Paid' AND c1.policy_no NOT IN (select policy_no from payment WHERE status='Pending' OR status='Scrap'))";
...
<div id="main">
      ...
      <h3 align="left">These are the list of policies which can be Extended</h3>
...
?> 
       <table width="828" border="1" cellpadding="5" cellspacing="0">
          ...
          echo "<tr>";
            echo "<td>".$count++."</td>";
            echo "<td><a href=\"extend_policy_form.php?policyno=".$row['policy_no']."\"target=\"_blank\">".$row['policy_no']."</td>";
            echo "<td>".$row['name']."</td>";
            echo "<td>".$row['type']."</td>";
            echo "<td></td>";
          echo "</tr>";
